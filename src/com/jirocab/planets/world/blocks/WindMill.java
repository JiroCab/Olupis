...
public class WindMill extends PowerGenerator {
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid){
        ...
        if(displayEfficiency && sumAttribute(attribute, x, y) != 0 && !Core.settings.getBool("olupis-debug")){
            drawPlaceText(Core.bundle.formatFloat("bar.efficiency", sumAttribute(attribute, x, y) * 100, 1), x, y, valid);
        }
        if (Core.settings.getBool("olupis-debug")) drawPlaceText(Core.bundle.formatFloat("bar.efficiency", ((attribute.env() + sumAttribute(attribute, x, y)) * powerProduction + powerProduction ) , 1),  x , y, valid);
    }
