...
public class LimitedLauncherSelect extends BaseDialog implements PlanetRenderer.PlanetInterfaceRenderer {
    ...
    public LimitedLauncherSelect(){
        ...
        addListener(new InputListener(){
            ...
            public boolean scrolled(InputEvent event, float x, float y, float amountX, float amountY){
                if(event.targetActor == LimitedLauncherSelect.this){
    ...
    boolean canSelect(Sector sector){
        if(mode == NyfMode.select){
            return sector.hasBase() && launchSector != null && sector.planet == launchSector.planet && (launchSector.near().contains(sector) || launchSector == sector);
        }
    ...
    public void renderSectors(Planet planet){
        ...
        if(state.uiAlpha > 0.01f){
            for(Sector sec : planet.sectors){
                if(canSelect(sec) || sec.unlocked() || debugSelect){
                    ...
                    if(color != null){
                        float mul = launchSector.near().contains(sec) ? 1 : 0.2f;
                        planets.drawSelection(sec, Tmp.c1.set(color).mul(0.8f).a(state.uiAlpha * mul), 0.026f, -0.001f);
                    }
                }else{
        ...
        if(state.uiAlpha > 0.001f){
            for(Sector sec : planet.sectors){
                ...
                if(sec.info.anyExports()){
                    planets.drawArc(planet, sec.tile.v, sec.info.destination.tile.v, Color.gray.write(Tmp.c2).a(state.uiAlpha), Pal.accent.write(Tmp.c3).a(state.uiAlpha), 0.4f, 90f, 25);
                }
            }
        }
    ...
    boolean selectable(Planet planet){
        ...
        if(mode == NyfMode.select) return planet == state.planet;
        ...
        stack(
                new Element(){
                    ...
                    public void act(float delta){
                        if(scene.getDialog() == LimitedLauncherSelect.this && !scene.hit(input.mouseX(), input.mouseY(), true).isDescendantOf(e -> e instanceof ScrollPane)){
                            scene.setScrollFocus(LimitedLauncherSelect.this);
                        }
                ...
                new Table(t -> {
                    ...
                    t.top();
                    t.label(() -> mode == NyfMode.select ? "@sectors.select" : "").style(Styles.outlineLabel).color(Pal.accent);
                }),
                buttons,
        ...
        if(hovered != null && !mobile && state.planet.hasGrid()){
            ...
            if(hovered != null){
                ...
                if(!canSelect(hovered)){
                    ...
                    }if(!launchSector.near().contains(hovered) && hovered.hasBase()){
                        tx.append("[gray]").append(Iconc.none).append(" ").append(Core.bundle.get("olupis-out-of-range")).append("\n[ ").append(hovered.name()).append(" ]");
                    }else{
    ...
    void updateSelected(){
        ...
        if((sector.hasBase() && mode == look) || canSelect(sector) || (sector.preset != null && sector.preset.alwaysUnlocked) || debugSelect){
            stable.button(
                            mode == NyfMode.select ? "@sectors.select" :
    ...
    void playSelected(){
        ...
        //save before launch.
        if(control.saves.getCurrent() != null && Vars.state.isGame() && mode != NyfMode.select){
            try{
